...
function calcRegister(data) {
  ...
  for (instruction of data) {
    ...
    switch (instruction[5]) {
      case "==":
        if (reg[instruction[4]] === parseInt(instruction[6])) {
          valid = true
        }
        break;
    ...
    if (instruction[1] === 'inc') {
      reg[instruction[0]] += parseInt(instruction[2]);
    } else {
      reg[instruction[0]] -= parseInt(instruction[2]);
    }
  }
  ...
  return reg[largest[1]];
}


function calcRegisterLargest(data) {
  let largest = -Infinity;
  data = data.split('\n').map(current => current.split(' '));
  for (instruction of data) {
    reg[instruction[0]] = 0;
...
console.log(calcRegister(input));
console.log(calcRegisterLargest(input));
