...
function hexMap(data) {
  ...
  return findDist(n, ne, nw, s, se, sw);
}



function hexMapMax(data) {
  data = data.split(',');
  ...
  for (mov of data) {
    ...
    let distance = findDist(n, ne, nw, s, se, sw);
    if (distance > largest) {
      largest = distance;
    }
  }
...
function findDist(n, ne, nw, s, se, sw) {
  while (true) {
    if (se > 0 && sw > 0) {
      se--;
      sw--;
...
console.log(hexMap(input));
console.log(hexMapMax(input));
