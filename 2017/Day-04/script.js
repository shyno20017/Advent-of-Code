...
let input = `kvvfl kvvfl olud wjqsqa olud frc
...
function isPassphraseValid(data) {
  data = data.split('\n');
...
function isPassphraseAnagramValid(data) {
  data = data.split('\n');
  ...
  for (elt of data) {
    ...
    for (var i = 0; i < words.length; i++) {
      for (var j = 0; j < words.length; j++) {
        if (areAnagrams(words[i], words[j]) && i !== j) {
          valid = false;
        }
      }
    }
...
console.log(isPassphraseValid(input));
console.log(isPassphraseAnagramValid(input));
